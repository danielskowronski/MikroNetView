...
def getApi(host: str, user: str, password: str):
    ...
    if _connection is None:
        ...
        _connection = routeros_api.RouterOsApiPool(
...
def dumbFetchLeases(host: str, user: str, password: str) -> list[LeaseInfo]:
    ...
    reginfos_raw = api.get_resource("/interface/wifi/registration-table").call(
        "print", {".proplist": RegInfoProps}
    )
    end_time = time.time()
    print(
        f"Fetched {len(reginfos_raw)} registration infos in {end_time - start_time:.3f} seconds"
    )
    reginfos = []
    ...
    end_time = time.time()
    print(f"Fetched {len(leases_raw)} leases in {end_time - start_time:.3f} seconds")
    leases = []
    start_time = time.time()
